...
function ProjectCard({ project, index }: { project: Project; index: number }) {
	...
	return (
		<motion.div variants={fadeInScale(index * 0.05)} className="h-full">
			<Card className="flex flex-col card-gradient">
...
export default function ProjectsPage() {
	// use your real projects array and chunk it into rows
	const rows: Project[][] = [];
	let start = 0;
	...
	// if you ever add more projects than rowConfigs cover,
	// put the leftovers in one last row
	if (start < projects.length) {
		rows.push(projects.slice(start));
	}
